...
public interface DocumentRepository extends Repository<Document, Long> {
    ...
    @Query(value = "SELECT * FROM slides WHERE sales_man_email = :email " +
            "AND status IN ('CREATED', 'UPDATED', 'BEFORE_AWS_S3_TRANSITION') AND slides.id_ai NOT IN (SELECT slide_id FROM customer_slide);"
            , nativeQuery = true)
    List<Document> findDocumentBySalesMan(@Param("email") String email);
